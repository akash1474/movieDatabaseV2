   ...
   static render(){
        ...
        this.bucket.forEach((item)=>{
                items+=`<div class="bucket__movie" data-id="${item.id}">
                                    <div class="movie__title">${item.title}</div>
                                    <div class="movie__size">${item.size>1024?(item.size/1024).toFixed(2)+' GB':item.size+' MB'}</div>
        ...
        const el=`<div class="bucket__header">
                            ...
                            <div class="bucket__stats">
                                <div class="stats__count stats">
                                    ...
                                    <p class="value">${this.bucket.length} Movies</p>
                                </div>
                                <div class="stats__size stats">
                                    <p class="title">Total Size:</p>
                                    <p class="value">${(totalSize/1024).toFixed(2)}GB</p>
                                </div>
                            </div>
    ...
    static addListeners(){
                    ...
                    Elements.bucketList.addEventListener("click",(e)=>{
                        ...
                        if((e.target! as Element).parentElement!.className==="bucket__movie" && id){
                            this.removeItem(+id);
                        }
                    });
                    this.hasRan=true;
